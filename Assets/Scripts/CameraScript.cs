...
public class CameraScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        /*  newPositionx = Mathf.SmoothDamp(transform.position.x, target.position.x, ref xVelocity, smoothTimex);
          ...
          */
        transform.position = new Vector3(Mathf.RoundToInt(target.position.x * divider) / divider, Mathf.RoundToInt(target.position.y * divider) / divider, transform.position.z);
    }
}
